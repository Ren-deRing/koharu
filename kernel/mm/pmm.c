...
int pmm_init() {
    ...
    boot_mmap_entry_t *mmap = g_boot_info->memory.entries;
    uint32_t count = g_boot_info->memory.count;
    uintptr_t array_phys = 0;

    for (size_t i = 0; i < count; i++) {
        if (mmap[i].type == MMAP_TYPE_USABLE) {
            uintptr_t start_addr = ALIGN_UP(mmap[i].base_addr, PAGE_SIZE);
            uintptr_t end_addr = ALIGN_UP(mmap[i].base_addr + mmap[i].length, PAGE_SIZE);
